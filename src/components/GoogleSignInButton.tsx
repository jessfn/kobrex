...
export function GoogleSignInButton({ label }: { label: string }) {
  return (
    <form action={signInWithGoogleAction}>
      <button
        type="submit"
        className="flex w-full items-center justify-center gap-2.5 rounded-lg border border-[var(--color-border-strong)] bg-white px-4 py-2.5 text-sm font-semibold text-[var(--foreground)] transition-all duration-150 hover:bg-[var(--color-surface-muted)] active:scale-[0.97]"
      >
